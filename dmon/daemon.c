 ...
 * DBus org.freedesktop.Notifications owner; forks doi(1) per notification.
 * Aggressively claims the name: replaces mako/dunst/anything else on startup
 * and re-asserts ownership on SIGHUP so stray daemons cannot sneak back.
 *
 * Build: make dmon
 */
#include <stdio.h>
...
/*
 * SIGHUP: re-assert DBus name ownership.  Useful when mako/dunst gets
 * accidentally started by something else and steals the name.
 * Send: kill -HUP $(pidof doid)
 */
static void sighup_handler(int sig) {
	...
	if (dbus_error_is_set(&err)) {
		log_write("sighup: dbus name re-assert failed: %s", err.message);
		dbus_error_free(&err);
	} else if (ret == DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		log_write("sighup: re-asserted " NOTIF_SERVICE);
	} else {
		log_write("sighup: name re-assert reply=%d", ret);
	}
}
...
static void slide_slots_down(int px, int py, int removed_index) {
	...
	for (i = 0; i < slot_count; i++) {
		...
		{ ssize_t _r = write(slots[i].write_fd, &reposition, sizeof(reposition)); (void)_r; }
	}
}
...
static DBusHandlerResult handle(DBusConnection *conn,
	...
	if (dbus_message_is_method_call(msg, NOTIF_SERVICE, "Notify")) {
...
reply:
	...
	} else if (dbus_message_is_method_call(msg, NOTIF_SERVICE, "GetCapabilities")) {
	...
	} else if (dbus_message_is_method_call(msg, NOTIF_SERVICE, "GetServerInformation")) {
	...
	} else if (dbus_message_is_method_call(msg, NOTIF_SERVICE, "CloseNotification")) {
		dbus_uint32_t id = 0;
		DBusError err;
		DBusMessage *r;
		dbus_error_init(&err);
...
int main(void) {
	...
	/* SIGHUP: re-assert DBus name (kick mako/dunst off the name) */
	memset(&sa, 0, sizeof(sa));
	...
	/*
	 * Claim org.freedesktop.Notifications with maximum aggression:
	 *   REPLACE_EXISTING — evicts mako, dunst, or any other owner
	 *   DO_NOT_QUEUE     — if someone else later steals it, we don't wait
	 *                      in a queue; they have to explicitly give it back
	 *
	 * To reclaim after an accidental steal: kill -HUP $(pidof doid)
	 */
	...
	if (ret != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		log_write("failed to own " NOTIF_SERVICE " (reply=%d)", ret);
		return EXIT_FAILURE;
	}
	...
	log_write("doid started, owning " NOTIF_SERVICE);
