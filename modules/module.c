...
static void hint_str(DBusMessageIter *dict, const char *key, const char *val) {
...
static void hint_int(DBusMessageIter *dict, const char *key, int ival) {
        DBusMessageIter entry, var;
        dbus_int32_t val = (dbus_int32_t)ival;
        dbus_message_iter_open_container(dict, DBUS_TYPE_DICT_ENTRY, NULL, &entry);
        dbus_message_iter_append_basic(&entry, DBUS_TYPE_STRING, &key);
        dbus_message_iter_open_container(&entry, DBUS_TYPE_VARIANT,
...
static dbus_uint32_t read_stored_id(const char *key) {
        ...
        while (fgets(line, sizeof(line), f)) {
                if (strncmp(line, key, klen) == 0 && line[klen] == '\t') {
                        dbus_uint32_t v = (dbus_uint32_t)atoi(line + klen + 1);
                        fclose(f);
                        return v;
                }
        }
...
static void store_id(const char *key, dbus_uint32_t id) {
        const char *home = getenv("HOME");
...
int doi_notify(const ModuleOpts *opts) {
        ...
        const char     *app     = "doi";
        const char     *summary = opts->summary ? opts->summary : "";
        const char     *body    = opts->body    ? opts->body    : "";
        const char     *icon    = opts->icon    ? opts->icon    : "";
        const char     *id_key  = summary[0]    ? summary       : "default";
        dbus_uint32_t   rep_id  = read_stored_id(id_key);
        dbus_uint32_t   out_id  = 0;
        dbus_int32_t    timeout = (dbus_int32_t)(opts->timeout > 0 ? opts->timeout : 5000);
